...
# parse dates — utc=True handles mixed timezone offsets cleanly
riders["signup_date"] = pd.to_datetime(riders["signup_date"], utc=True)
trips["pickup_time"]  = pd.to_datetime(trips["pickup_time"],  utc=True)
trips["dropoff_time"] = pd.to_datetime(trips["dropoff_time"], utc=True)
...
print(agg["churned"].value_counts())
